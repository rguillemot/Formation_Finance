...
\usetheme{Warsaw}
\title[Produits dérivés de taux]{Ajustements de convexité et exotiques de première génération. }
...
\begin{frame}{LIBOR \it{in arrears}}
\begin{figure}[h]
...
\[
\begin{split}
R_A&=\mathbb{E}_t^{Q^{T_2}}[L(T^f,T_1,T_2)]\\
&=L(t,T_1,T_2)\\
\end{split}
\]
...
\[
\begin{split}
R_B&=\mathbb{E}_t^{Q^{T_1}}[L(T^f,T_1,T_2)]\\
&=\mathbb{E}_t^{Q^{T_2}}\Big[L(T^f,T_1,T_2)\frac{1+\delta L(T^f,T_1,T_2)}{1+\delta L(t,T_1,T_2)}\Big]\\
&=\frac{L(t,T_1,T_2)+\delta \mathbb{E}_t^{Q^{T_2}}[L(T^f,T_1,T_2)^2]}{1+\delta L(t,T_1,T_2)}
\end{split}
\]
\end{frame}
...
\hline
ARREARS & $\frac{\delta(F-K^*)}{(1+T_1 R)}$ & $H_1=\frac{-T_1 \delta (F-K^*)}{(1+ T_1 R)^2(1+\delta F)}$ &  $H_2=\frac{\delta}{(1+ T_1 R)}$\\
\hline
& & Ratio MM & Ratio FRA\\
\hline
&& $r_1=\frac{\delta(F-K^*)}{(1+\delta F)}$ & $r_2=\frac{ (1+\delta F)^2}{1+\delta K^*}$\\
\hline
\end{tabular}\\
\normalsize
\vspace{0.5cm}
