...
\item \onslide<3->\textbf{Funded Credit Derivatives}: le vendeur de protection détient un actif. Exemple: Asset Swap.
\end{itemize}
\vspace{0.5cm}
...
\begin{overprint}
\onslide<1>A la mise en place du swap, l'acheteur livre l'obligation au vendeur en échange de sa valeur faciale.
\onslide<2>Par la suite l'acheteur réçoit le coupon de l'obligation de la part du vendeur en échange du taux \textbf{LIBOR+x bps}, c'est à dire 100.
...
\vspace{0.5cm}
\end{overprint}
\onslide<4>La Jambe fixe a $N^F$ flux et la jambe variable $N^V$ flux. Elle maturent simultanément en $T=T^F_{N^F}=T^V_{N^V}$
\end{frame}
...
\end{align*}
\end{overprint}
\onslide<4> On constate dans la pratique que ce prix est plus élévé que le prix de marché. On appelle CDS basis:\\ 
\[
